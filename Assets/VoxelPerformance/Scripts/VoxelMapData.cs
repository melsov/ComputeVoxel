...
namespace VoxelPerformance
{
    ...
    public class VoxelMapData
    {
        ...
        public VoxelMapData(ComputeShader shader, VGenConfig vGenConfig) {
            perlinGen = shader;
        ...
        void initTemporaryBuffers() {
            MapVoxels = new ComputeBuffer(vGenConfig.ChunkPerlinGenArraySize, sizeof(int)); //  64 * 256 * 256, sizeof(int));  // each int stores four voxels
            MapHeights = new ComputeBuffer(vGenConfig.ColumnsPerChunk, sizeof(int)); //  256 * 256, sizeof(int)); // don't compute heights. we don't want to make terrain
        ...
        public void setMapOffset(Vector3 mapoffset) {
            this.mapOffset = Vector3.Scale(mapoffset, vGenConfig.ChunkSize); //mapoffset * 256;
        ...
        void callPerlinMapGenKernel() {
            perlinGen.Dispatch(PerlinMapGenKernel, vGenConfig.GroupsPerChunkX, vGenConfig.GroupsPerChunkY, vGenConfig.GroupsPerChunkZ); //   8, 1, 8);
        }
