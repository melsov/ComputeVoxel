...
namespace g3
{
    public struct Vector3f : IComparable<Vector3f>, IEquatable<Vector3f>
    {
        ...
        public Vector3f negToZeroPosToOne()
        {
            return new Vector3f(
                x > 0f ? 1f : 0f, 
                y > 0f ? 1f : 0f, 
        ...
        public Vector3f posToZeroNegToOne()
        {
            return new Vector3f(
                x < 0f ? 1f : 0f, 
                y < 0f ? 1f : 0f, 
        ...
        public Vector3f posToOneNegToNegOne()
        {
            return new Vector3f(
                x < 0f ? -1f : 1f, 
                y < 0f ? -1f : 1f, 
        ...
        public int IndexOfAbsMin {
            get {
                int result = Mathf.Abs(x) < Mathf.Abs(y) ? 0 : 1;
                float compare = result == 0 ? x : y;
                return compare < Mathf.Abs(z) ? result : 2;
            }
        }

        public CardinalDirection DirectionOfAbsMin {
            get {
                return GetDirection(IndexOfAbsMin);
            }
        }

        public Vector3f isolateComponent(int index)
        {
            switch (index)
            {
                case 0: default: return this * AxisX;
                case 1: return this * AxisY;
                case 2: return this * AxisZ;
            }
        }

        public enum CardinalDirection { Nowhere, Up, Down, Left, Right, Forward, Backward }

        public CardinalDirection GetDirection(int index)
        {
            switch (index)
            {
                case 0: default: return x < 0f ? CardinalDirection.Left : CardinalDirection.Right;
                case 1: return y < 0f ? CardinalDirection.Down : CardinalDirection.Up;
                case 2: return z < 0f ? CardinalDirection.Backward : CardinalDirection.Forward;
            }
        }

        public static Vector3f ZeroSafeDivide(Vector3f a, Vector3f b)
        {
            return new Vector3f(
                SafeDivide.ZeroSafeDivide(a.x, b.x),
                SafeDivide.ZeroSafeDivide(a.y, b.y),
                SafeDivide.ZeroSafeDivide(a.z, b.z)
                );
        }
        ...
        static float Rand () { return UnityEngine.Random.Range(99999999f, 99999999f); }

        public static Vector3f GetRandom()
        {
            return new Vector3f(Rand(), Rand(), Rand());
        }

        public static Vector3f GetRandomNormalized() { return GetRandom().Normalized; }
        ...
        public IEnumerable<Vector3f> IteratorXY(Vector2f incr)
        {
            foreach(var v in IteratorXY(incr, Vector2f.Zero)) { yield return v; }
        }

        public IEnumerable<Vector3f> IteratorXYZ(Vector3f incr)
        {
            foreach(var v in IteratorXYZ(incr, Zero)) { yield return v; }
        }

        public IEnumerable<Vector3f> IteratorXY(Vector2f incr, Vector2f start)
        {
            float xx = start.x; float yy = start.y;
            for(xx = 0; xx < x; xx += incr.x)
            {
                for(yy = 0; yy < y; yy += incr.y)
                {
                    yield return new Vector3f(xx, yy, z);
                }
            }
        }

        public IEnumerable<Vector3f> IteratorXYZ(Vector3f incr, Vector3f start)
        {
            float xx, yy, zz;
            xx = start.x; yy = start.y; zz = start.z;
            for (xx = 0; xx < x; xx += incr.x)
            {
                for (yy = 0; yy < y; yy += incr.y)
                {
                    for(zz = 0; zz < z; zz+= incr.z)
                        yield return new Vector3f(xx, yy, z);
                }
            }
        }
