...
Shader "VoxelPerformance/_MipVoxelGeometryShader" 
{
  Properties
  {
  ...
  SubShader
  {
    ...
    StructuredBuffer<data> _displayPoints;
      ...
      uint pos = _displayPoints[id].voxel;
        ...
        o.pos = float4( pos/65536 % CHUNK_DIM_X, pos/256 % CHUNK_DIM_Y, pos % CHUNK_DIM_Z, 1.0f );

      uint voxelColor = pos/16777216 % 256;

      //test colors
      //o._color = float4( sin(o.pos.xyz * float3(32, 1, 64)/128 * PI ) / 2 + .5, 1);

      //o._color = (voxelColor > 0) * _Color1 + ((voxelColor < 1) * _Color);
      ...
      [maxvertexcount(12)]
      void geom( triangle inputGS p[3], inout TriangleStream<input> triStream )
      {
      ...
      input pIn1, pIn2, pIn3; //, pIn4;
        ...
        pIn1.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, p[0].pos ));
        triStream.Append( pIn1 );

        pIn2.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, p[1].pos  ));
        triStream.Append( pIn2 );

        pIn3.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, p[2].pos ));
        triStream.Append( pIn3 );

        triStream.RestartStrip();

//         shift = (_cameraPosition.x < voxelPosition.x)?float4( 1, 1, 1, 1 ):float4( -1, 1, -1, 1 );
//         // float mipShift = (pos.x < (CHUNK_DIM_X - 1.1)) * max(_mipStretch - 2, 0);

//         float4 mipPos = pos;
//         mipPos.x += _mipStretch / 2 - .5;

// //TODO: 
// //keep border voxels normal, other off of the border boxels
// //also, just make a separate function in order to clamp the pos:

//         halfXPos = _Size * max(_mipStretch * .9, 0) / 2;
//         halfXPos = halfXPos * shift.x + mipPos.x;
//         halfXPos = clamp(halfXPos, 0, CHUNK_DIM_X);
//         halfXPos -= mipPos.x;
//         halfXPos = abs(halfXPos);

//         halfXNeg = -_Size * max(_mipStretch * .9,  0) / 2;
//         halfXNeg = halfXNeg * shift.x + mipPos.x;
//         halfXNeg = clamp(halfXNeg, 0, CHUNK_DIM_X);
//         halfXNeg -= mipPos.x;
//         halfXNeg = -abs(halfXNeg);

//         float isMiddle = (pos.x < (CHUNK_DIM_X - 1.1)) * (pos.x > .1);
//         pos = (1 - isMiddle) * pos + isMiddle * mipPos; 

//         pIn1.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, pos + shift*float4( halfXNeg, -halfS, halfS, 0 ) ));
//         triStream.Append( pIn1 );

//         pIn2.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, pos + shift*float4( halfXNeg, halfS, halfS, 0 ) ));
//         triStream.Append( pIn2 );

//         pIn3.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, pos + shift*float4( halfXNeg, -halfS, -halfS, 0 ) ));
//         triStream.Append( pIn3 );

//         pIn4.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, pos + shift*float4( halfXNeg, halfS, -halfS, 0 ) ));
...
//         pIn3.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, pos + shift*float4( halfXPos, -halfS, halfS, 0 ) ));
//         triStream.Append( pIn3 );

//         pIn4.pos = mul( UNITY_MATRIX_VP, mul( _worldMatrixTransform, pos + shift*float4( halfXPos, -halfS, -halfS, 0 ) ));
//         triStream.Append( pIn4 );
